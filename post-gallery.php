<?php
    ...
    if(isset($_POST['initiative'])) {
        ...
        $i = 0;
        while (file_exists("graphics/data/gallery/$initiative/$image_name")) {
        ...
        if (!file_exists("graphics/data/gallery/$initiative")) {
            mkdir("graphics/data/gallery/$initiative");
        }
        $url = "graphics/data/gallery/$initiative/$image_name";

        $filename = compress_image($_FILES["imageData"]["tmp_name"], $url, 50);

        $query = "INSERT INTO `gallery` (`src`, `alt`, `initiative`, `category`) VALUES ('$filename', 'Image', '$initiative', 'slider')";
